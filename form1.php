...
document.getElementById("utm_source1").value = params.get("utm_source") || "organic";
document.getElementById("utm_medium1").value = params.get("utm_medium") || "organic";
document.getElementById("utm_campaign1").value = params.get("utm_campaign") || "organic";
</script>
<?php
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($response_data->success) || !$response_data->success) {
        die("Verification failed: Google reCAPTCHA check did not pass.");
    }
    ...
    $url = "https://apps.cratiocrm.com/Customize/Webhooks/webhook.php?id=840231";
    ...
    $utm_source=trim($_POST['utm_source1'] ?? '');
    $utm_medium=trim($_POST['utm_medium1'] ?? '');
    $utm_campaign=trim($_POST['utm_campaign1T'] ?? '');
    ...
    try {
        ...
        $mail->Password = '@eNfAXrdA7LxD@d'; // ⚠️ Replace with real password or app password
        ...
        // Email content
        $mail->isHTML(true);
        $mail->Subject = "Home Page Contact Form Submission";
        $mail->Body = "
